...
二。关于通信过程
    ...
    当一个Peer的listen收到一个心跳包时，它首先会用心跳包的信息更新自己的数据
    库，之后它会检查这个心跳包中有没有自己这个IP：PORT，如果有则结束，如果没有则
    把自己的信息加入协议体，泛洪。
        我的计划是在启动了UDPserver之后启动一个线程用于发送心跳包，三个时候会发送
    心跳包：新的udpserver启动时，收到心跳包时（通过把自己附加在旧的心跳包尾的方式
    ），好久没有进行上述两种操作时。由于涉及到进程间通信，留给我来写吧。
    4.泛洪法发送Post
        编号：2,字符串：FF，数据报体：
        [时间,哈希,父哈希,内容]
